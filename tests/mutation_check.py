...
MUTATIONS = [
    ...
    # -- reachability by BODY on a dereferenced document, both directions ---
    (
        "a dereferenced document goes back to having no reachable schema at all",
        "apidrift/loader.py",
        "    hit = index.get(json.dumps(node, sort_keys=True))",
        "    hit = None",
        ["test_a_dereferenced_removal_still_inlined_unchanged_is_not_a_break",
         "test_a_dereferenced_removal_only_on_a_removed_operation_is_not_a_break"],
    ),
    (
        "body matching fires on every node, inventing reachability",
        "apidrift/loader.py",
        "    hit = index.get(json.dumps(node, sort_keys=True))",
        "    hit = [n for names in index.values() for n in names]",
        ["test_a_dereferenced_body_that_appears_nowhere_is_unreachable"],
    ),
    (
        "body matching is applied to a document that DOES link its schemas",
        "apidrift/loader.py",
        "    if not every and views:",
        "    if views:",
        ["test_body_matching_is_NOT_applied_to_a_document_that_links"],
    ),
    (
    ...
    (
        ...
        ["test_inlining_a_DIFFERENT_shape_is_still_a_break",
         "test_inlining_a_DIFFERENT_item_shape_is_still_a_break"],
    ),
    # -- reaching a schema through an array's ITEMS, both directions -------
    (
        "shape-at-parents stops seeing a schema at an array's items",
        "apidrift/diff.py",
        "            if field.type != f\"->{name}\" and field.item != f\"->{name}\":",
        "            if field.type != f\"->{name}\":",
        ["test_a_schema_inlined_at_an_ARRAYS_ITEMS_is_not_a_break"],
    ),
    (
        "shape-at-parents compares every field, related or not",
        "apidrift/diff.py",
        "            if field.type != f\"->{name}\" and field.item != f\"->{name}\":\n                continue",
        "            if False:\n                continue",
        ["test_an_unrelated_field_changing_in_the_parent_is_not_this_break"],
    ),
    (
        "an inline items enum is flattened away to plain \"string\" again",
        "apidrift/loader.py",
        "                    item_enum=_item_enum(node, resolver) if not target else None,",
        "                    item_enum=None,",
        ["test_the_shape_projection_carries_an_INLINE_items_enum",
         "test_a_schema_inlined_at_an_ARRAYS_ITEMS_is_not_a_break"],
    ),
    (
        "the shape projection ignores the element enum it was given",
        "apidrift/diff.py",
    ...
    # -- the notation escape for a pure inlining, both directions ----------
    (
        "the notation escape is dropped (a pure inlining reads as a break)",
        "apidrift/diff.py",
        "            if (not view.required\n                    and _field_notation(replacement) == _view_notation(view)):",
        "            if False:",
        ["test_a_named_array_inlined_is_not_a_break_when_its_ELEMENT_changed"],
    ),
    (
        "the notation escape fires without comparing notations",
        "apidrift/diff.py",
        "            if (not view.required\n                    and _field_notation(replacement) == _view_notation(view)):",
        "            if True:",
        ["test_inlining_a_named_array_as_a_DIFFERENT_array_is_still_a_break"],
    ),
    (
    ...
    # -- direction: a whole schema travels ONE way, both directions ---------
    (
        "the name test goes back to merging request and response names",
        "apidrift/diff.py",
        "        side = (\"response\" if response_only\n                else \"request\" if old.used_in_requests(name)\n                and not old.used_in_responses(name) else \"all\")",
        "        side = \"all\"",
        ["test_replacing_a_request_body_outright_is_still_a_break"],
    ),
    (
        "a request body 'renamed' to the operation's RESPONSE schema",
        "apidrift/diff.py",
    ...
    (
        "the name test can no longer see the new operation's names at all",
        "apidrift/diff.py",
        "        visible_new = (by_direction or {}).get((\"new\", side)) or new_names",
        "        visible_new = {}",
        ["test_a_request_schema_relocated_WITHIN_the_request_is_not_a_break"],
    ),
    (
    ...
    (
        ...
        ["test_an_inlined_schema_is_refuted",
         "test_a_dereferenced_body_still_inlined_is_refuted"],
    ),
    (
        "the checker drops the dereferenced-document branch",
        "tests/measure_precision.py",
        "        linked = len(ref_sites(old, None))\n        if linked == 0:",
        "        linked = len(ref_sites(old, None))\n        if False:",
        ["test_a_dereferenced_document_with_no_signal_at_all_is_UNDECIDABLE",
         "test_a_dereferenced_body_that_CHANGED_is_confirmed"],
    ),
    # -- the checker's body-identity rule, both directions ------------------
    (
        "the checker abstains on a dereferenced document again",
        "tests/measure_precision.py",
        "            return check_schema_removed_dereferenced(\n                name, old_schemas[name], old, new)",
        "            return UNDECIDABLE, \"dereferenced, no signal\"",
        ["test_a_dereferenced_body_still_inlined_is_refuted",
         "test_a_dereferenced_body_only_on_a_removed_operation_is_refuted",
         "test_a_dereferenced_schema_carried_by_no_operation_is_refuted"],
    ),
    (
        "the checker drops the control on body identity (refutes with no signal)",
        "tests/measure_precision.py",
    ...
    (
        "the checker calls every inline use site intact",
        "tests/measure_precision.py",
        "        if here is not _MISSING and _canon(here) == _canon(old_body):",
        "        if here is not _MISSING or True:",
        ["test_a_dereferenced_body_that_CHANGED_is_confirmed"],
    ),
    (
        "the checker stops crediting the removal of the whole operation",
        "tests/measure_precision.py",
